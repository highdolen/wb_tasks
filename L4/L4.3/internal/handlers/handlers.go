...
func (h *Handlers) CreateEvent(w http.ResponseWriter, r *http.Request) {
	...
	event, err := h.calendar.CreateEventWithReminder(
		req.UserID,
		req.Date,
		req.Title,
		req.Reminder,
...
// ArchivedEvents - возвращает архивные события пользователя.
func (h *Handlers) ArchivedEvents(w http.ResponseWriter, r *http.Request) {
	userIDStr := r.URL.Query().Get("user_id")
	if userIDStr == "" {
		sendError(w, "missing user_id", http.StatusBadRequest)
		return
	}

	userID, err := strconv.Atoi(userIDStr)
	if err != nil {
		sendError(w, "invalid user_id", http.StatusBadRequest)
		return
	}

	events := h.calendar.GetArchivedEvents(userID)
	sendJSON(w, models.EventsResponse{Result: events})
}
...
func decodeRequest(w http.ResponseWriter, r *http.Request, v interface{}) error {
	...
	case *models.CreateEventRequest:
		...
		req.Reminder = r.FormValue("reminder")
...
func (h *Handlers) SetupRoutes() *mux.Router {
	...
	router.HandleFunc("/archived_events", h.ArchivedEvents).Methods("GET")
